...
export default class TcTrainingListView extends NavigationMixin(LightningElement) {
    ...
    wiredUser({ error, data }) {
        if (data) {
            ...
            this.showToast('Error', error.body?.message || 'Failed to load user data', 'error');
        }
    }
    ...
    columns = [
        ...
        { label: 'Course', fieldName: 'hed__Course__r', type: 'text',
        ...
        { label: 'Primary Trainer', fieldName: 'hed__Faculty__r.Name', type: 'text' },
        { label: 'Secondary Trainer', fieldName: 'cc_Secondary_Faculty__r.Name', type: 'text' },
    ...
    wiredTrainings(result) {
        ...
        if (result.data) {
            ...
            this.showToast('Error', result.error.body?.message || 'Failed to load trainings', 'error');
        }
    }
    ...
    get formattedTrainings() {
        ...
        return this.trainings.map(training => {
            return {
                ...
                primaryTrainerName: training.hed__Faculty__r ? training.hed__Faculty__r.Name : '',
                primaryTrainerId: training.hed__Faculty__c || '',
                finalizedStatus: training.Finalized__c ? 'Yes' : 'No',
                finalizedClass: training.Finalized__c ? 'status-finalized' : 'status-not-finalized',
    ...
    get paginationText() {
        ...
        const end = Math.min(this.computedOffset + this.pageSize, this.totalRecords);
    ...
    handlePrintAction(event) {
        const recordId = event.currentTarget.dataset.id;
        // Add print logic here
        console.log('Print action for record:', recordId);
   }
    ...
    async handleViewCertificateAction(event) {
        ...
        const courseName = training && training.courseName ? training.courseName : 'Certificate';
        // Format current date and time for filename
        const now = new Date();
        const pad = n => n.toString().padStart(2, '0');
        const dateStr = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`;
        const timeStr = `${pad(now.getHours())}_${pad(now.getMinutes())}_${pad(now.getSeconds())}`;
        const fileName = `${courseName} Certificate ${dateStr} ${timeStr}.pdf`;

        this.isLoading = true;
        try {
            const result = await generateCertificate({ trainingId });
            if (result && Array.isArray(result) && result.length > 0 && result[0].documentBase64) {
